...
Ead mozziead_env_p(MOZZI_AUDIO_RATE);
...
Ead mozziead_env_v(MOZZI_AUDIO_RATE);
long node_vca_out = 0;
...
void updateControl() {
    ...
    // Parameter update for audio node env_p
        if((long)node_clock_out>0){ mozziead_env_p.start((unsigned int)(long)5, (unsigned int)(long)100); }
    ...
    // Parameter update for audio node env_v
        if((long)node_clock_out>0){ mozziead_env_v.start((unsigned int)(long)10, (unsigned int)(long)400); }
}
