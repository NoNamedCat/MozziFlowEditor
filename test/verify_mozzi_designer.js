...
global.Rpd = { allNodeTypes: {}, VERSION: "2.1.1", events: { filter: () => ({ onValue: () => {} }) } };
...
loadDesignerScript('nodes/mozzinodes.js');
loadDesignerScript('examples.js');
...
global.mozziFlowClassConnection.push({
    ...
    inlet_alias: 'audio_in',
    outlet_alias: 'out'
});

const code = NodeToMozziCpp();
assert(code.indexOf('mozzisin_') !== -1, "Connected oscillator should be in code");
assert(code.indexOf('mozzisaw_') === -1, "Orphan oscillator should be REMOVED (Optimized)");
assert(code.indexOf('updateAudio') !== -1, "Sketch should have updateAudio");

// 3. SAMPLE PROCESSING & HUFFMAN
console.log("\n3. Audio Processing:");
...
osc.data = { sampleInfo: { tableString: "1,2,3", data: [1,2,3], info: "Mock", cpp: "const int8_t PROGMEM sample[] = {1,2,3};" } };
const sampleCode = NodeToMozziCpp();
assert(sampleCode.indexOf('PROGMEM') !== -1, "Sample data should be in PROGMEM");
assert(sampleCode.indexOf('mozzisample_') !== -1, "Sample object should be instantiated");
...
Object.keys(global.EXAMPLES).forEach(key => {
    try {
        global.mozziFlowContainer = [];
        global.mozziFlowClassConnection = [];
        global.importPlainNetwork(global.EXAMPLES[key]);
        const exCode = NodeToMozziCpp();
        assert(exCode.length > 100, `Example '${key}' should generate valid code`);
    } catch(e) {
        assert(false, `Example '${key}' failed to import/export: ${e.message}`);
    }
});

// 5. ANALOG NODE LOGIC
console.log("\n5. Analog Node Logic:");
const asyncNode = global.NodeLibrary.find(n => n.nodetype === 'input/mozzi_async_analog');
assert(asyncNode !== undefined, "Async Analog node should exist");
if (asyncNode) {
    const code = asyncNode.mozzi.control({}, "test_v", { pin: "A0" });
    assert(code.indexOf('mozziAnalogRead') !== -1, "Async node should use mozziAnalogRead");
    assert(asyncNode.mozzi.includes.indexOf('#include <mozzi_analog.h>') !== -1, "Async node should include mozzi_analog.h");
}

const arduinoNode = global.NodeLibrary.find(n => n.nodetype === 'input/arduino_analog');
assert(arduinoNode !== undefined, "Arduino Analog node should exist");
if (arduinoNode) {
    const code = arduinoNode.mozzi.control({}, "test_v", { pin: "A0" });
    assert(code.indexOf('analogRead') !== -1 && code.indexOf('mozziAnalogRead') === -1, "Arduino node should use standard analogRead");
}
